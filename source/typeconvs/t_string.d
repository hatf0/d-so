...
void str_to_flt(ref decompiler dec) {
	dec.stacks.f_s ~= popOffStack(dec.stacks.s_s);
}

void str_to_uint(ref decompiler dec) {
	dec.stacks.i_s ~= popOffStack(dec.stacks.s_s);
}

void str_to_none(ref decompiler dec) {
	if(dec.stacks.l_s[3] == opcode.OP_CALLFUNC_RESOLVE || (dec.stacks.l_s[3] == opcode.DECOMPILER_ENDIF && dec.stacks.l_s[3] == opcode.OP_CALLFUNC_RESOLVE))
	{
		string writeOut = addTabulation(popOffStack(dec.stacks.s_s), dec.indentation); 
		if(writeOut[writeOut.length - 1] != ';') {
			writeOut ~= ";";
		}
		dec.writeln(writeOut);
	}
	else {
...
void advance_str_comma(ref decompiler dec) {
	dec.stacks.s_s ~= popOffStack(dec.stacks.s_s) ~ ",";
}

void advance_str_nul(ref decompiler dec) {

}

void advance_str(ref decompiler dec) {

}

void advance_str_appendchar(ref decompiler dec) {
	dec.stacks.s_s ~= popOffStack(dec.stacks.s_s) ~ cast(char)dec.fi.code[dec.i];
	dec.i++;
}

void rewind_str(ref decompiler dec) {
...
void terminate_rewind_str(ref decompiler dec) {

}

void compare_str(ref decompiler dec) {
	string rhs = popOffStack(dec.stacks.s_s);
	string lhs = popOffStack(dec.stacks.s_s);
